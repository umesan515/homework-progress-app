  ...
  const createThread = async () => {
    ...
    try {
      ...
      const r = await (async () => {
        ...
        if (!res.ok) throw new Error(`API ${res.status}: POST ${url} --- response --- ${txt}`);
    ...
    <div className="p-6 space-y-4">
      ...
      <div className="rounded-xl border bg-gray-50 p-4 space-y-3">
        ...
        <div className="grid grid-cols-3 gap-2">
          ...
          <input className="rounded-lg border px-3 py-2" placeholder="chapterId（任意）" value={chapterId} onChange={(e) => setChapterId(e.target.value)} />
