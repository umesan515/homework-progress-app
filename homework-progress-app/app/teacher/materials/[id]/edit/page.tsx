...
export default function TeacherMaterialEditPage() {
  ...
  useEffect(() => {
    if (!ready || !materialId) return;
    apiGet<MaterialRow>(`/teacher/materials/${encodeURIComponent(materialId)}`)
      .then((row) => {
        setForm({
          title: row.title ?? "",
          ...
          interactive_kind: (row.interactive_kind ?? "linear") as InteractiveKind,
          interactive_config_text: JSON.stringify(row.interactive_config ?? {}, null, 2),
          is_published: !!row.is_published,
          class_ids: Array.isArray(row.class_ids) ? row.class_ids : [],
        });
      })
      .catch((e: any) => setErr(String(e?.message ?? "教材の取得に失敗しました。")));
  }, [ready, materialId]);
  ...
  const onSubmit = async () => {
    ...
    setErr(null);
    try {
      await apiPut(`/teacher/materials/${encodeURIComponent(materialId)}`, {
        title: form.title,
    ...
    <main className="page-shell space-y-6">
      <div className="space-y-3">
        <div className="flex flex-wrap items-center gap-2 text-xs text-slate-500">
          ...
          <span className="chip">編集</span>
        </div>
        <div className="flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
          <div className="space-y-2">
            <h1 className="page-title">教材を編集</h1>
        ...
        <div className="space-y-6">
          ...
          <section className="surface-card p-5 sm:p-6">
            ...
            <div className="mt-5 flex flex-wrap items-center gap-3 rounded-3xl border border-slate-200 bg-slate-50 px-4 py-3">
              ...
              <span className="text-xs text-slate-500">一度非公開で保存し、授業準備が整ってから公開する運用もできます。</span>
            </div>
